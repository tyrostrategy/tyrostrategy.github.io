...
export default function ProfilPage() {
  ...
  const handleLocaleChange = (newLocale: "tr" | "en") => {
    ...
    if (dbUser) {
      ...
      toast.success("Dil tercihi güncellendi", { message: newLocale === "en" ? "English" : "Türkçe" });
    }
  };
    ...
    <div className="max-w-2xl mx-auto py-8 px-4 space-y-5">
      ...
      <div className="rounded-2xl overflow-hidden bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
        ...
        <div className="relative overflow-hidden px-6 pb-5 pt-6" style={{ background: sidebarTheme.bg }}>
          ...
          <div className="relative z-10 flex items-center gap-5">
            {/* Avatar */}
            <div className="w-[72px] h-[72px] rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg shrink-0"
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-[20px] font-bold leading-tight" style={{ color: sidebarTheme.textPrimary ?? "#fff" }}>
                {currentUser.name}
              </h1>
              <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                <span className="flex items-center gap-1 text-[12px] font-medium" style={{ color: sidebarTheme.textSecondary ?? "rgba(255,255,255,0.7)" }}>
                  <Mail size={12} /> {currentUser.email}
                </span>
              </div>
              <div className="flex items-center gap-2 mt-2">
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold text-white/90"
                  style={{ backgroundColor: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)" }}>
                  <Shield size={11} /> {currentUser.role}
                </span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium text-white/70"
        ...
        <div className="grid grid-cols-3 divide-x divide-tyro-border/15">
          <StatCell icon={<Briefcase size={15} />} value={myProjeCount} label="Projelerim" color={accentColor} />
          <StatCell icon={<ListChecks size={15} />} value={myAksiyonCount} label="Aksiyonlarım" color="#3b82f6" />
          <StatCell icon={<Trophy size={15} />} value={myAchievedCount} label="Tamamlanan" color="#10b981" />
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
        <div className="px-5 py-3 border-b border-tyro-border/15">
          <h2 className="text-[13px] font-bold text-tyro-text-primary">Hesap Bilgileri</h2>
        </div>
        <div className="divide-y divide-tyro-border/10">
          <DetailRow label="Ad Soyad" value={currentUser.name} />
          <DetailRow label="E-posta" value={currentUser.email} />
          <DetailRow label="Departman" value={currentUser.department || "—"} />
          <DetailRow label="Yetki Rolü" value={currentUser.role} badge badgeColor={roleColor} />
          {dbUser?.createdAt && <DetailRow label="Kayıt Tarihi" value={formatDate(dbUser.createdAt)} />}
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_2px_16px_rgba(0,0,0,0.04)] overflow-hidden">
        <div className="px-5 py-3 border-b border-tyro-border/15">
          <h2 className="text-[13px] font-bold text-tyro-text-primary">Tercihler</h2>
        </div>
        <div className="px-5 py-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-[13px] font-semibold text-tyro-text-primary">Uygulama Dili</p>
